...
class FoodGatheringMultiEnv(gym.Env, BasicInterface):
    ...
    def __init__(self, render = False):
        ...
        BasicInterface.__init__(self, render = render, lidar_points = self.lidar_points, world_size = 1, view_camera_distance = 2.1, view_camera_angle = -89.9)

        self.action_space       = spaces.Box(low=-1.0, high=1.0, shape=(self.robots_count*2,), dtype=numpy.float32)
        self.observation_space  = spaces.Box(low=-1.0, high=1.0, shape=(self.robots_count, 7, self.lidar_points), dtype=numpy.float32)
    ...
    def _update_observation(self):
        result    = numpy.zeros((self.robots_count, 7, self.lidar_points), dtype=numpy.float32)
        for i in range(self.robots_count):
    ...
    def _update_observation_robot(self, robot_id, lidar_points):
        ...
        result    = numpy.zeros((7, lidar_points), dtype=numpy.float32)
        ...
        result[6] = lidar[0]        #other robots lidar
